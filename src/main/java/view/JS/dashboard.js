...
function assignationLivraison() {  
  ...
  if (donneesGlobales && donneesGlobales.sites) {
    extraireEtAfficherLivraisons(donneesGlobales.sites);
  }
}

function extraireEtAfficherLivraisons(sites) {
  ...
  const livraisons = Array.from(livraisonsMap.values())
    .filter(liv => liv.collecte && liv.depot);
  
  afficherLivraisonsDisponibles(livraisons);
}
...
function afficherLivraisonsDisponibles(livraisons) {
  ...
  if (livraisons.length === 0) {
    container.innerHTML = '<p class="empty-state">Aucune livraison disponible</p>';
    return;
  }
...
function creerElementLivraison(livraison) {
  ...
  div.innerHTML = `
    <div style="display:flex; align-items:center; gap:8px;">
      <strong style="font-size:1.1em; color:#667eea;">L${livraison.id}</strong>
      <div style="flex:1; font-size:0.85em; color:#666;">
        <div style="margin:2px 0;">
          <span style="color:#38a169;">📍</span> ${collecteLat}, ${collecteLng}
        </div>
        <div style="margin:2px 0;">
          <span style="color:#e53e3e;">📦</span> ${depotLat}, ${depotLng}
        </div>
      </div>
    </div>
  `;
...
function creerZoneLivreur(numero) {
  ...
  dropzone.addEventListener('drop', (e) => {
    ...
    const livraisonId = e.dataTransfer.getData('text/plain');
